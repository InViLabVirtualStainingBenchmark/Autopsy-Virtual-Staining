...
| `train_stage2_seperate_train_by_iters.py` | `tc.epoch_begin = 79` → `tc.epoch_begin = args.epoch_begin` (default 0); added `--epoch_begin` arg | Was hardcoded to original researcher's resume epoch; `range(79, 2)` = empty so training never ran |
| `train_stage2_seperate_train_by_iters.py` | `--train_data`/`--val_data` changed from glob pattern to directory path; glob built inside Python as `os.path.join(dir, f'*.{ext}')` | Shell was expanding `*.png` before Python received it, returning 0 images |
| `train_stage2_seperate_train_by_iters.py` | Added `--ext` arg (default `png`); used to build glob pattern | Same reason as test_G.py — needed for MIST-HER2 `.jpg` images |
